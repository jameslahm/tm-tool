...
exports.saveTemplate = exports.readTemplate = exports.unregisterTemplate = exports.registerTemplate = exports.verifyUrl = exports.resolveTemplate = exports.TEMPLATEPATH = void 0;
...
var readTemplate = function () {
  try {
    var templateMap_1 = JSON.parse(
      fs_1['default'].readFileSync(
        path_1['default'].resolve(__dirname, './config/template.json'),
        'utf-8'
      )
    );
    Object.keys(templateMap_1).forEach(function (k) {
      if (templateMap_1[k].type === 'local') {
        templateMap_1[k].url = path_1['default'].resolve(
          __dirname,
          templateMap_1[k].url
        );
      } else {
  ...
  try {
    if (templateMap[templateName].type === 'local') {
      shelljs_1['default'].cp(
        '-r',
        path_1['default'].resolve(__dirname, templateMap[templateName].url),
        projectName
      );
    } else {
      shelljs_1['default'].exec(
        'git clone ' + templateMap[templateName].url + ' ' + projectName
      );
    }
...
var verifyUrl = function (tempateUrl) {
  ...
  if (
    validURLS.findIndex(function (validUrl) {
      return validUrl.test(tempateUrl);
    }) !== -1
  ) {
  ...
  if (templateMap.hasOwnProperty(templateName)) {
    ...
    console.log(
      chalk_1['default'].green(
        'Did you mean ' + searchRes[searchRes.length - 1].item.name
      )
    );
  }
  saveTemplate(templateMap);
...
var TEMPLATEPATH = path_1['default'].resolve(__dirname, './templates');
