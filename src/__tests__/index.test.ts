...
test('should unregister react-typescript-webpack',()=>{
  const orginalTemplateMap:templateMapType=JSON.parse(fs.readFileSync(path.resolve(__dirname,'../../template.json'),'utf-8'))

  shell.exec('tm-cli unregister react-typescript-webpack')
  const templateMap:templateMapType=JSON.parse(fs.readFileSync(path.resolve(__dirname,'../../template.json'),'utf-8'))
